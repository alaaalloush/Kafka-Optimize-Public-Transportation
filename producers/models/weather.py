...
class Weather(Producer):
    ...
    def __init__(self, month):
        ...
        #
        #
        topic_name = f"com.transitchicago.weather.current_status"
        super().__init__(
            topic_name,
        ...
        # Define key schema in `schemas/weather_key.json 
        if Weather.key_schema is None:
            with open(f"{Path(__file__).parents[0]}/schemas/weather_key.json") as f:
                Weather.key_schema = json.load(f)

        # Define value schema in `schemas/weather_value.json
    ...
    def run(self, month):
        ...
        logger.info(f"Sent data to Kafka topic {self.topic_name} via REST Proxy {json.dumps(resp.json(), indent=2)}")
